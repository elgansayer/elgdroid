...
public final class ActivityThread {
    ...
    private void handleBindApplication(AppBindData data) {
      ...
      Log.e(TAG, " ELGAN SAYER HERE ");

        //AppDisplayBackend backend = AppDisplayBackend.getInstance();
        ...
        try {
            ...
            String pkgname = data.appInfo.processName;
            AppDisplayOpsManager.AppDisplayRow mAppSettings = dooda.getAppDisplayRow(pkgname);

            int dpiValue = mAppSettings.getDPI();
            int fontScaleValue = mAppSettings.getFontScale();

            Log.e(TAG, "ELGAN SAYER dpiValue " + dpiValue + " fontScaleValue " + fontScaleValue + " for " + data.appInfo.processName);

            if (dpiValue > 0) {
                data.config.densityDpi = dpiValue;
            }
            if (fontScaleValue != data.config.fontScale) {
                data.config.fontScale = fontScaleValue;
            }
        } catch (RemoteException ex) {
